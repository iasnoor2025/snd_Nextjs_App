    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div>
            <span className="font-medium">{t('view.type')}:</span> {record.type}
          </div>
          <div>
            <span className="font-medium">{t('view.status')}:</span> {record.status}
          </div>
          <div>
            <span className="font-medium">{t('view.date')}:</span>{' '}
            {record.scheduled_date ? new Date(record.scheduled_date).toLocaleString() : '-'}
          </div>
          <div>
            <span className="font-medium">{t('view.due')}:</span>{' '}
            {record.due_date ? new Date(record.due_date).toLocaleDateString() : '-'}
          </div>
          <div>
            <span className="font-medium">{t('view.title')}:</span> {record.title}
          </div>
          <div>
            <span className="font-medium">{t('view.description')}:</span>{' '}
            {record.description || '-'}
          </div>
          <div>
            <span className="font-medium">{t('view.itemsUsed')}:</span>
          </div>
          <ul className="list-disc pl-5">
            {(record.items || []).map((it: any) => (
              <li key={it.id}>
                {it.name} — {Number(it.quantity).toFixed(2)} x {Number(it.unit_cost).toFixed(2)} ={' '}
                {Number(it.total_cost).toFixed(2)}
              </li>
            ))}
          </ul>
          <div className="font-medium">
            {t('view.totalCost')}: {Number(record.cost || 0).toFixed(2)}
          </div>
              ...
              <div className="flex justify-end">
                <Button
                  onClick={async () => {
                    ...
                    await ApiService.post(`/maintenance/${id}/items`, payload, {
