...
import { CheckCircle, Clock, Eye, RefreshCw, Timer, UserX, XCircle } from 'lucide-react';
...
export function TimesheetsSection({
  ...
  const getNextApprovalStage = (currentStatus: string) => {
    switch (currentStatus) {
      case 'draft':
        return t('timesheet.approvalWorkflow.submitDraft');
      case 'submitted':
        return t('timesheet.approvalWorkflow.foreman');
      case 'foreman_approved':
        return t('timesheet.approvalWorkflow.incharge');
      case 'incharge_approved':
        return t('timesheet.approvalWorkflow.checking');
      case 'checking_approved':
        return t('timesheet.approvalWorkflow.manager');
      default:
        return t('timesheet.approvalWorkflow.finalApproved');
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('dashboard.timesheets.todaysAttendance')}
            </CardTitle>
            <CardDescription>
            ...
            <PermissionBased action="approve" subject="Timesheet">
              <div className="flex items-center gap-2">
                <Badge variant="outline">
                  ...
                  {t('dashboard.timesheets.canApprove')}
                </Badge>
                <div className="text-xs text-muted-foreground">
                  {t('dashboard.timesheets.role')}: {session?.user?.role?.replace('_', ' ')}
                </div>
              </div>
            </PermissionBased>
            <Button
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-5 gap-4">
          ...
          <div className="text-center p-3 rounded-lg border bg-card">
            ...
            <div className="text-sm text-muted-foreground">{t('dashboard.timesheets.status.absent')}</div>
          </div>
          <div className="text-center p-3 rounded-lg border bg-card">
            ...
            <div className="text-sm text-muted-foreground">
              {t('dashboard.timesheets.status.approved')}
            </div>
          </div>
          <div className="text-center p-3 rounded-lg border bg-card">
            ...
            <div className="text-sm text-muted-foreground">{t('dashboard.timesheets.status.pending')}</div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-lg border bg-muted/50">
          <div className="flex items-center justify-between mb-3">
            <div className="text-sm font-medium">{t('dashboard.timesheets.approvalWorkflow')}</div>
            <PermissionBased action="approve" subject="Timesheet">
              <div className="text-xs text-muted-foreground">
                {t('dashboard.timesheets.role')}: {session?.user?.role?.replace('_', ' ')}
              </div>
          ...
          <div className="flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              ...
              <span>{t('dashboard.timesheets.workflowStages.draftSubmitted')}</span>
            </div>
            <div className="text-muted-foreground">→</div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-primary rounded-full"></div>
              <span>{t('dashboard.timesheets.workflowStages.foreman')}</span>
            </div>
            <div className="text-muted-foreground">→</div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-primary rounded-full"></div>
              <span>{t('dashboard.timesheets.workflowStages.incharge')}</span>
            </div>
            <div className="text-muted-foreground">→</div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-primary rounded-full"></div>
              <span>{t('dashboard.timesheets.workflowStages.checking')}</span>
            </div>
            <div className="text-muted-foreground">→</div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-green-500 rounded-full"></div>
              <span className="text-green-600">{t('dashboard.timesheets.workflowStages.manager')}</span>
            </div>
          </div>
        </div>
              ...
              {safeTimesheetData.slice(0, 10).map(item => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    ...
                    >
                      {item.status === 'present'
                        ? t('timesheet.attendance.present')
                        : item.status === 'late'
                          ? t('timesheet.attendance.late')
                          : item.status === 'half-day'
                            ? t('timesheet.attendance.halfDay')
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        item.approvalStatus === 'manager_approved'
                          ? 'default'
                          : item.approvalStatus === 'foreman_approved'
                            ? 'secondary'
                            : item.approvalStatus === 'incharge_approved'
                              ? 'outline'
                              : item.approvalStatus === 'checking_approved'
                                ? 'secondary'
                    ...
                    >
                      {item.approvalStatus === 'manager_approved'
                        ? t('timesheet.approvalWorkflow.finalApproved')
                        : item.approvalStatus === 'foreman_approved'
                          ? t('timesheet.approvalWorkflow.foremanApproved')
                          : item.approvalStatus === 'incharge_approved'
                            ? t('timesheet.approvalWorkflow.inchargeApproved')
                            : item.approvalStatus === 'checking_approved'
                              ? t('timesheet.approvalWorkflow.checkingApproved')
                              : item.approvalStatus === 'submitted'
                                ? t('timesheet.approvalWorkflow.submitted')
                                : item.approvalStatus === 'draft'
                                  ? t('timesheet.approvalWorkflow.draft')
                                  : item.approvalStatus}
                    </Badge>
                    {item.approvalStatus !== 'manager_approved' && (
                      ...
                      <div className="font-medium text-blue-600 dark:text-blue-400">
                        {(() => {
                          try {
                            ...
                            const otHours = Number(item.overtimeHours) || 0;
                            const total =
                              (isNaN(hours) ? 0 : hours) + (isNaN(otHours) ? 0 : otHours);
                      ...
                      {item.approvalStatus !== 'manager_approved' && (
                        <PermissionBased action="approve" subject="Timesheet">
                          <Button
                            ...
                            title={t('timesheet.approveToNextStage', {
                              stage: getNextApprovalStage(item.approvalStatus),
                            })}
              ...
              <div className="mt-3 pt-3 border-t">
                ...
                <div className="grid grid-cols-5 gap-2 text-xs">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-muted-foreground">
                      {t('timesheet.approvalWorkflow.foreman')}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium">
                    ...
                    <div className="text-muted-foreground">
                      {t('timesheet.approvalWorkflow.incharge')}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium">
                    ...
                    <div className="text-muted-foreground">
                      {t('timesheet.approvalWorkflow.checking')}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium">
                    ...
                    <div className="text-muted-foreground">
                      {t('timesheet.approvalWorkflow.manager')}
                    </div>
                  </div>
                </div>
              </div>
