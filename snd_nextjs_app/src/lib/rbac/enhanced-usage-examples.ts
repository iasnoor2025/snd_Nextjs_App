...
export class PermissionExamples {
  ...
  static async checkBasicPermission(userId: string) {
    ...
    const canApproveTimesheets = await enhancedPermissionService.can(userId, 'approve', 'timesheet');
    
    // Check if user can approve at foreman stage
    const canApproveForeman = await enhancedPermissionService.can(userId, 'approve', 'timesheet.foreman');
  ...
  static async checkRoles(userId: string) {
    ...
    const isApprover = await enhancedPermissionService.hasAnyRole(userId, ['MANAGER', 'ADMIN', 'SUPER_ADMIN']);
  ...
  static async checkApprovalWorkflow(userId: string, approvalStage: string) {
    ...
    const canApprove = await enhancedPermissionService.can(userId, 'approve', `timesheet.${approvalStage}`);
  ...
  static async checkBulkOperations(userId: string) {
    ...
    const canPerformBulkOperations = await enhancedPermissionService.canAny(userId, bulkPermissions);
