...
export class PDFGenerator {
  ...
  private static generateQuotation(doc: any, quotationData: QuotationData): void {
    ...
    doc.text('Equipment Rental & Construction Services', pageWidth / 2, yPosition, {
      align: 'center',
    });
    ...
    doc.text(
      `Date: ${new Date(quotationData.createdAt).toLocaleDateString()}`,
      margin,
      yPosition + 15
    );
    doc.text(
    ...
    if (quotationData.projectName || quotationData.startDate) {
      ...
      if (quotationData.startDate) {
        doc.text(
          `Start Date: ${new Date(quotationData.startDate).toLocaleDateString()}`,
          margin,
          yPosition
        );
        yPosition += 12;
      ...
      if (quotationData.expectedEndDate) {
        doc.text(
          `End Date: ${new Date(quotationData.expectedEndDate).toLocaleDateString()}`,
          margin,
          yPosition
        );
        yPosition += 12;
    ...
    const summaryItems = [
      ...
      {
        label: `Tax (${quotationData.tax}%):`,
        value: `SAR ${parseFloat(quotationData.taxAmount).toFixed(2)}`,
      },
    ...
    summaryItems.forEach((item, index) => {
      ...
      doc.text(item.value, summaryBoxX + summaryBoxWidth - 10, yPosition + index * 12, {
        align: 'right',
      });
    });
    ...
    yPosition += 8;
    doc.text(
      'Contact: info@cat-international.com | Tel: +966-XX-XXX-XXXX',
      pageWidth / 2,
      yPosition,
      { align: 'center' }
    );
  ...
  private static generateRentalInvoice(doc: any, invoiceData: RentalInvoiceData): void {
    ...
    if (invoiceData.projectName || invoiceData.startDate) {
      ...
      if (invoiceData.startDate) {
        doc.text(
          `Start Date: ${new Date(invoiceData.startDate).toLocaleDateString()}`,
          margin,
          yPosition
        );
        yPosition += 12;
      ...
      if (invoiceData.expectedEndDate) {
        doc.text(
          `End Date: ${new Date(invoiceData.expectedEndDate).toLocaleDateString()}`,
          margin,
          yPosition
        );
        yPosition += 12;
    ...
    const summaryItems = [
      ...
      {
        label: `Tax (${invoiceData.tax}%):`,
        value: `SAR ${parseFloat(invoiceData.taxAmount).toFixed(2)}`,
      },
  ...
  // Add compact signature section on the same page
  private static addCompactSignatureSection(
    doc: any,
    ...
    const customerFields = ['Name:', 'Position:', 'Signature:', 'Date:'];
    ...
    const companyFields = ['Authorized By:', 'Position:', 'Signature:', 'Date:'];
