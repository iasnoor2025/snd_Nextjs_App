...
export class ERPNextFinancialService {
    ...
    try {
      ...
      if (selectedMonth) {
        ...
        if (year && month && !isNaN(year) && !isNaN(month)) {
          ...
          console.log('📅 Using selected month:', selectedMonth, 'Target:', targetMonth.toISOString())
        } else {
      ...
      const accountsEndpoint = '/api/resource/Account?limit_page_length=1000&fields=["name","account_name","account_type","parent_account","root_type","report_type","account_currency","is_group"]';
      ...
      const targetMonthEnd = new Date(targetMonth.getFullYear(), targetMonth.getMonth() + 1, 0).toISOString().split('T')[0];
      const previousMonthStart = previousMonth.toISOString().split('T')[0];
      const previousMonthEnd = new Date(previousMonth.getFullYear(), previousMonth.getMonth() + 1, 0).toISOString().split('T')[0];
      ...
      if (accountsData?.data) {
        ...
        for (const account of accountsData.data) {
          if (account.is_group === 0) { // Only leaf accounts
            ...
            if (targetMonthGL?.data) {
              targetMonthGL.data.forEach((entry: any) => {
                if (entry.account === account.name) {
                  targetMonthBalance += (parseFloat(entry.debit) || 0) - (parseFloat(entry.credit) || 0);
                }
              });
            }
            ...
            if (previousMonthGL?.data) {
              previousMonthGL.data.forEach((entry: any) => {
                if (entry.account === account.name) {
                  previousMonthBalance += (parseFloat(entry.debit) || 0) - (parseFloat(entry.credit) || 0);
                }
              });
            }
            ...
            if (normalizedRootType.includes('income') || normalizedRootType.includes('revenue') || normalizedRootType.includes('sales')) {
            ...
            } else if (normalizedRootType.includes('expense') || normalizedRootType.includes('cost') || normalizedRootType.includes('purchase')) {
        ...
        try {
          ...
          if (broaderGL?.data && broaderGL.data.length > 0) {
            ...
            broaderGL.data.forEach((entry: any) => {
              ...
              if (entryDate >= targetMonth && entryDate <= new Date(targetMonth.getFullYear(), targetMonth.getMonth() + 1, 0)) {
      ...
      if (result.totalIncome === 0 && result.totalExpenses === 0) {
        console.log('⚠️ All approaches resulted in zero values, using invoice fallback as last resort')
        return await this.getFinancialOverviewFromInvoices(selectedMonth);
      }
  ...
  private static async getFinancialOverviewFromInvoices(selectedMonth?: string): Promise<FinancialOverview> {
    try {
      ...
      try {
        ...
        console.log('📊 Previous month purchase invoices:', previousMonthPurchase?.data?.length || 0)
      } catch (error) {
      ...
      if (targetMonthSales?.data) {
        targetMonthSales.data.forEach((invoice: any) => {
          ...
          if (status === 'paid' || status === 'submitted' || outstanding === 0 || outstanding < amount * 0.1) {
            targetMonthIncome += amount;
        ...
        try {
          ...
          if (historicalSales?.data && historicalSales.data.length > 0) {
            ...
            const recentInvoices = historicalSales.data
              .filter((invoice: any) => {
                const invoiceDate = new Date(invoice.posting_date);
                return invoiceDate >= targetMonth && invoiceDate <= new Date(targetMonth.getFullYear(), targetMonth.getMonth() + 1, 0);
              })
  ...
  static async testERPNextData(): Promise<any> {
    try {
      ...
      // Test accounts
      try {
        const accountsEndpoint = '/api/resource/Account?limit_page_length=10&fields=["name","account_name","account_type","root_type"]';
        const accountsData = await this.makeERPNextRequest(accountsEndpoint);
      ...
      // Test GL entries
      try {
        const glEndpoint = '/api/resource/GL Entry?limit_page_length=10&fields=["account","debit","credit","posting_date"]';
        const glData = await this.makeERPNextRequest(glEndpoint);
      ...
      // Test sales invoices
      try {
        const salesEndpoint = '/api/resource/Sales Invoice?limit_page_length=10&fields=["name","grand_total","posting_date","status"]';
        const salesData = await this.makeERPNextRequest(salesEndpoint);
        results.salesInvoices.count = salesData?.data?.length || 0;
        results.salesInvoices.sample = salesData?.data?.slice(0, 3) || [];
        console.log('✅ Sales Invoices test successful:', results.salesInvoices.count, 'invoices found')
      } catch (error) {
      ...
      try {
        const purchaseEndpoint = '/api/resource/Purchase Invoice?limit_page_length=10&fields=["name","grand_total","posting_date","status"]';
        ...
        console.log('✅ Purchase Invoices test successful:', results.purchaseInvoices.count, 'invoices found')
      } catch (error) {
