...
import { and, or, ilike, eq, gte, lte, lt, inArray, isNull, sql, asc } from 'drizzle-orm';
...
export const GET = withReadPermission(
  async (request: NextRequest) => {
    try {
      ...
      const windowExpr = range === 'past'
        ? and(gte(employeesTable.iqamaExpiry, start.toISOString()), lte(employeesTable.iqamaExpiry, now.toISOString()))
        : and(gte(employeesTable.iqamaExpiry, now.toISOString()), lte(employeesTable.iqamaExpiry, end.toISOString()));
