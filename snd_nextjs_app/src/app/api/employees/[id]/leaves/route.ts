...
const getEmployeeLeavesHandler = async (_request: NextRequest, ...args: unknown[]) => {
  try {
...
const createEmployeeLeaveHandler = async (request: NextRequest, ...args: unknown[]) => {
  try {
    const { params } = args[0] as { params: Promise<{ id: string }> };
    const resolvedParams = await params;
...
export const GET = withPermission(PermissionConfigs.leave.read)(getEmployeeLeavesHandler);
export const POST = withPermission(PermissionConfigs.leave.create)(createEmployeeLeaveHandler);
