...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to initialize cron service', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to get cron service status', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
