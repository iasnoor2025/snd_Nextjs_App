...
export async function GET(
  ...
  try {
    ...
    const payslipData = {
      ...
      attendanceData: generateAttendanceData(payroll),
      company: {
...
function generateAttendanceData(payroll: any) {
  const month = new Date(payroll.year, payroll.month - 1, 1);
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    attendanceData.push({
      ...
      hours: isWeekend ? 0 : 8,
      overtime: isWeekend ? 0 : (day % 7 === 0 ? 2 : 0) // Some overtime on certain days
    });
  }
