...
export default function CreateEmployeePage() {
  ...
  const fetchNextFileNumber = async (isExternal: boolean, autoUpdate: boolean = false) => {
    try {
      const url = `/api/employees/next-file-number${isExternal ? '?external=true' : ''}`;
      const res = await fetch(url);
      if (res.ok) {
        const data = await res.json();
        if (data?.success && data?.next) {
          const nextFileNumber = String(data.next);
          setSuggestedFileNumber(nextFileNumber);

          // Auto-update form only if:
          // 1. File number is empty, OR
          // 2. autoUpdate flag is true (when toggling external status)
          if (autoUpdate) {
            setFormData(prev => {
              const currentFileNumber = prev.fileNumber || '';
              // Update if empty or if it looks like an auto-generated number (numeric or EXT###)
              const isAutoGenerated = !currentFileNumber ||
                /^(EXT)?[0-9]+$/.test(currentFileNumber);

              if (!currentFileNumber || isAutoGenerated) {
                return { ...prev, fileNumber: nextFileNumber };
              }
              return prev; // Keep user-entered custom file numbers
            });
          } else if (!formData.fileNumber) {
            // Only auto-fill if field is empty on initial load
            setFormData(prev => ({ ...prev, fileNumber: nextFileNumber }));
          }
        }
      }
    } catch {
  ...
  // When external status changes, update file number suggestion
  useEffect(() => {
    if (formData.is_external !== undefined) {
      fetchNextFileNumber(formData.is_external, true); // autoUpdate = true when toggling
    }
          ...
          <Card>
            <CardContent className="p-6">
              <div className="text-center">
                ...
                <Link href={`/${locale}/employee-management`}>
            ...
            <TabsContent value="salary" className="mt-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {formData.is_external
                      ? 'Salary information is optional for external employees'
                      : t('employee.create.salary.description')
                    }
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="basic_salary">
                        {t('employee.fields.basicSalary')} {!formData.is_external && <span className="text-red-500">*</span>}
                      </Label>
                      <Input
                        id="basic_salary"
                        type="number"
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={checked => {
                          handleInputChange('is_external', checked);
                          if (!checked) {
