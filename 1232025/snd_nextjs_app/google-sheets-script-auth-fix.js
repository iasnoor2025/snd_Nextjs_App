// Add this simple function to your Google Apps Script to trigger authorization
function authorizeExternalRequests() {
  try {
    ...
    console.log('Authorization successful! Response code:', response.getResponseCode());
    SpreadsheetApp.getUi().alert('✅ Authorization Successful!', 
      'External requests are now authorized. You can now update the database from Google Sheets!', 
      SpreadsheetApp.getUi().ButtonSet.OK);
  } catch (error) {
    console.log('Authorization needed:', error.message);
    SpreadsheetApp.getUi().alert('🔐 Authorization Required', 
      'Please run this function again and grant permissions when prompted.', 
      SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
...
function onOpen() {
  const ui = SpreadsheetApp.getUi();
  ui.createMenu('Timesheet Data (Fixed)')
    .addItem('🔐 Authorize External Requests', 'authorizeExternalRequests')
...
function showEditInstructions() {
  ...
  ui.alert(
    ...
    '🔐 If you get permission errors, use the "Authorize External Requests" menu option first.\n' +
