 ...
 * 
 * This script moves the migrated Supabase files to the correct folder structure
 * based on employee file numbers instead of database IDs
 */
...
console.log('📁 Move Files to Correct Folders Script');
console.log('=======================================\n');
...
async function moveFilesToCorrectFolders() {
  try {
    ...
    const filesToMove = await db.execute(sql`
      SELECT 
        ed.id,
        ed.employee_id,
        ed.file_name,
    ...
    if (filesToMove.rows.length === 0) {
      console.log('✅ No files found with incorrect folder structure!');
      return;
    }
    
    console.log(`Found ${filesToMove.rows.length} files to move:`);
    
    const filesToProcess: FileToMove[] = filesToMove.rows.map((row: any) => {
      const currentPath = row.file_path.replace('https://minio.snd-ksa.online/employee-documents/', '');
      const fileName = row.file_name;
      const newPath = `employee-${row.file_number}/${fileName}`;
    ...
    for (let i = 0; i < filesToProcess.length; i++) {
      ...
      console.log(`\n📄 Processing ${i + 1}/${filesToProcess.length}: ${file.fileName}`);
    ...
    if (successCount > 0) {
      ...
      console.log('The application will now find files in the correct folder structure.');
    }
...
// Run the move
moveFilesToCorrectFolders().catch(error => {
